...
void otcbook::resumeorder(const name& owner, const name& order_side, const uint64_t& order_id) {
    ...
    CHECKC( (order_status_t)order.status == order_status_t::PAUSED, err::STATUS_ERROR, "order not paused" );
    order_wrapper_ptr->modify(_self, [&]( auto& row ) {
...
void otcbook::closeorder(const name& owner, const name& order_side, const uint64_t& order_id) {
    ...
    merchant_t merchant(owner);
    CHECKC( _dbc.get(merchant), err::RECORD_NOT_FOUND, "merchant not found: " + owner.to_string() );
    CHECKC( ORDER_SIDES.count(order_side) != 0, err::DATA_ERROR, "Invalid order side" );
    ...
    const auto &order = order_wrapper_ptr->get_order();
    CHECKC( owner == order.owner,err::NO_AUTH, "have no access to close others' order");
    CHECKC( (uint8_t)order.status != (uint8_t)order_status_t::CLOSED,err::STATUS_ERROR, "order already closed" );
    CHECKC( order.va_frozen_quantity.amount == 0,err::DATA_ERROR, "order being processed" );
    CHECKC( order.va_quantity >= order.va_fulfilled_quantity,err::DATA_ERROR, "order quantity insufficient" );
...
void otcbook::_opendeal( const name& taker, const name& order_side, const uint64_t& order_id,
    ...
    auto conf = _conf();
    CHECKC( conf.status == conf_status::RUNNING, err::STATUS_ERROR, "service is in maintenance" );
    CHECKC( ORDER_SIDES.count(order_side) != 0,err::STATUS_ERROR, "Invalid order side" );
    ...
    CHECKC( deal_quantity <= order.va_max_take_quantity,err::DATA_ERROR, "Order's max accept quantity not met!" );
...
deal_t otcbook::_closedeal(const name& account, const uint8_t& account_type, const uint64_t& deal_id, const string& close_msg, const bool& by_transfer) {
    ...
    CHECKC( (uint8_t)status != (uint8_t)deal_status_t::CLOSED,err::STATUS_ERROR, "deal already closed: " + to_string(deal_id) );
    CHECKC( (uint8_t)status != (uint8_t)deal_status_t::CANCELLED,err::STATUS_ERROR, "deal already cancelled: " + to_string(deal_id) );
    ...
    case account_type_t::MERCHANT:
        ...
        CHECKC( (uint8_t)status == (uint8_t)deal_status_t::MAKER_RECV_AND_SENT || 
            (uint8_t)status == (uint8_t)deal_status_t::TAKER_SENT,err::STATUS_ERROR, "can only close deal in status taker_sent or maker_recv");
        CHECKC( by_transfer || (merchant_paid_at + seconds(_conf().payed_timeout) < current_time_point()),err::DATA_ERROR, "deal is not expired.");
        break;
    default:
        CHECKC(false, err::PARAM_ERROR, "account type not supported: " + to_string(account_type));
        break;
    }
    ...
    CHECKC( order.va_frozen_quantity >= deal_quantity,err::DATA_ERROR, "Err: order frozen quantity smaller than deal quantity" );
...
void otcbook::canceldeal(const name& account, const uint8_t& account_type, const uint64_t& deal_id, bool is_taker_black) {
    ...
    case account_type_t::USER:
        switch ((deal_status_t) status) {
            ...
            case deal_status_t::MAKER_ACCEPTED: {
                ...
                CHECKC(merchant_accepted_at + seconds(_conf().accepted_timeout) < now, err::DATA_ERROR, "deal is not expired.");
                break;
            }
            default:
    ...
    case account_type_t::ARBITER:
        ...
        break;
    default:
        CHECKC(false,err::DATA_ERROR, "account type not supported: " + to_string(account_type));
        break;
    }
...
deal_t otcbook::_process(const name& account, const uint8_t& account_type, const uint64_t& deal_id, uint8_t action_type) {
    ...
    CHECKC( status != deal_status_t::CLOSED, err::STATUS_ERROR,"deal already closed: " + to_string(deal_id) );
    CHECKC( status != deal_status_t::CANCELLED,err::STATUS_ERROR, "deal already cancelled: " + to_string(deal_id) );
...
#define DEAL_ACTION_CASE(_action, _limited_account_type, _limit_arbit_status,  _limited_status, _next_status) \
    ...
    if (limited_status != deal_status_t::NONE)
        CHECKC(limited_status == status,err::STATUS_ERROR, "can not process deal action:" + to_string((uint8_t)action_type)
             + " at status: " + to_string((uint8_t)status) );
    if (limited_account_type != account_type_t::NONE)
        CHECKC(limited_account_type == (account_type_t)account_type,err::STATUS_ERROR,
    ...
    if ( (uint8_t)limit_arbit_status != (uint8_t)arbit_status_t::NONE)
        CHECKC(arbit_status == limit_arbit_status,err::STATUS_ERROR,
            "can not process deal action:" + to_string((uint8_t)action_type)
...
void otcbook::startarbit(const name& account, const uint8_t& account_type, const uint64_t& deal_id) {
    ...
    auto status = (deal_status_t)deal_itr->status;
    auto arbit_status = (arbit_status_t)deal_itr->arbit_status;
    CHECKC( arbit_status == arbit_status_t::UNARBITTED , err::STATUS_ERROR, "arbit already started: " + to_string(deal_id) );
    ...
    CHECKC( can_arbit_status.count(status) != 0, err::STATUS_ERROR,"status illegal: " + to_string((uint8_t)status) );
...
void otcbook::closearbit(const name& account, const uint64_t& deal_id, const uint8_t& arbit_result) {
    ...
    const auto &order_maker  = deal_itr->order_maker;
    CHECKC( arbit_status == arbit_status_t::ARBITING, err::STATUS_ERROR, "arbit isn't arbiting: " + to_string(deal_id) );
...
void otcbook::cancelarbit( const uint8_t& account_type, const name& account, const uint64_t& deal_id )
{
    ...
    CHECKC( arbit_status == arbit_status_t::ARBITING,err::STATUS_ERROR, "deal is not arbiting" );
    auto status = deal_itr->status;

    switch ((account_type_t) account_type) {
    case account_type_t::MERCHANT:
        CHECKC( deal_itr->order_maker == account, err::NO_AUTH, "maker account mismatched");
        CHECKC( status == (uint8_t)deal_status_t::MAKER_ACCEPTED, err::STATUS_ERROR,
...
void otcbook::resetdeal(const name& account, const uint64_t& deal_id){
    ...
    auto status = (deal_status_t)deal_itr->status;
    CHECKC( status != deal_status_t::CLOSED,err::STATUS_ERROR, "deal already closed: " + to_string(deal_id) );
    CHECKC( status != deal_status_t::CREATED, err::STATUS_ERROR, "deal no need to reverse" );
...
void otcbook::withdraw(const name& owner, asset quantity){
    ...
    CHECKC(state >= merchant_status_t::BASIC || state == merchant_status_t::DISABLED,err::STATUS_ERROR, "merchant not enabled");
    ...
    CHECKC( (time_point_sec(current_time_point())-merchant.updated_at) > limit_seconds,err::DATA_ERROR,
...
void otcbook::_deposit(name from, name to, asset quantity, string memo) {
    ...
    CHECKC((merchant_status_t)merchant.status >= merchant_status_t::BASIC,err::STATUS_ERROR,
        "merchant not enabled");
...
const fiat_conf_t& otcbook::_conf(bool refresh/* = false*/) {
    ...
    CHECKC(_gstate.conf_contract.value != 0,err::DATA_ERROR, "Invalid conf_table");
...
void otcbook::_sub_balance(merchant_t& merchant, const asset& quantity, const string & memo){
    CHECKC( merchant.assets[quantity.symbol].balance >= quantity.amount,err::DATA_ERROR, "merchant stake balance quantity insufficient");
    merchant.assets[quantity.symbol].balance -= quantity.amount;
    merchant.updated_at = current_time_point();
    _dbc.set( merchant , get_self());
    if(memo.length() > 0) STAKE_CHANGED(merchant.owner, -quantity, memo);
...
void otcbook::_unfrozen(merchant_t& merchant, const asset& quantity){
    CHECKC( merchant.assets[quantity.symbol].frozen >= quantity.amount,err::DATA_ERROR, "merchant stake frozen quantity insufficient");
    merchant.assets[quantity.symbol].frozen -= quantity.amount;
    merchant.assets[quantity.symbol].balance += quantity.amount;
    merchant.updated_at = current_time_point();
    _dbc.set( merchant , get_self());
...
void otcbook::_merchant_apply(name from, asset quantity, vector<string_view> memo_params) {
    ...
    merchant_t merchant(from);
    CHECKC(!_dbc.get( merchant ),err::RECORD_EXISTING,"merchant is existed");
