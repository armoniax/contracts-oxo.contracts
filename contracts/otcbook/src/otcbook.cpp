...
void otcbook::setmerchant( const name& sender, const merchant_info& mi ) {
    ...
    CHECKC(is_account(mi.account),err::RECORD_NOT_FOUND,  "account invalid: " + mi.account.to_string());
    CHECKC(mi.merchant_name.size() < 32,err::PARAM_ERROR, "merchant_name size too large: " + to_string(mi.merchant_name.size()) );
    CHECKC(mi.email.size() < 64,err::PARAM_ERROR, "email size too large: " + to_string(mi.email.size()) );
    CHECKC(mi.merchant_detail.size() < 255,err::PARAM_ERROR, "mechant detail size too large: " + to_string(mi.merchant_detail.size()) );
    CHECKC(mi.memo.size() < max_memo_size,err::PARAM_ERROR, "memo size too large: " + to_string(mi.memo.size()) );
    CHECKC(mi.reject_reason.size() < 255,err::PARAM_ERROR, "reject reason size too large: " + to_string(mi.memo.size()) );

    auto merchant = merchant_t(mi.account);
    auto found = _dbc.get(merchant);
    CHECKC( found, err::RECORD_FOUND, "merchant not existing: " + mi.account.to_string() )
...
void otcbook::remerchant( const merchant_info& mi) {
    ...
    auto merchant = merchant_t(mi.account);
    auto found = _dbc.get(merchant);
    CHECKC( found, err::RECORD_FOUND, "merchant not found: " + mi.account.to_string() )
...
void otcbook::openorder(const name& owner, const name& order_side, const set<name> &pay_methods, const asset& va_quantity, const asset& va_price,
    ...
    CHECKC(_conf().status == conf_status::RUNNING,err::PARAM_ERROR, "service is in maintenance");
    require_auth( owner );
    CHECKC( ORDER_SIDES.count(order_side) != 0,err::PARAM_ERROR, "Invalid order side" );
    CHECKC( va_quantity.is_valid(),err::PARAM_ERROR, "Invalid quantity");
    CHECKC( va_price.is_valid(),err::PARAM_ERROR, "Invalid va_price");
    const auto& conf = _conf();
    CHECKC( va_price.symbol == conf.fiat_type,err::PARAM_ERROR, "va price symbol not allow");
    CHECKC( conf.coin_as_stake.count(va_quantity.symbol),err::PARAM_ERROR, "va quantity symbol hasn't config stake asset");
    if (order_side == BUY_SIDE) {
        CHECKC( conf.buy_coins_conf.count(va_quantity.symbol) != 0,err::PARAM_ERROR, "va quantity symbol not allowed for buying" );
    } else {
        CHECKC( conf.sell_coins_conf.count(va_quantity.symbol) != 0,err::PARAM_ERROR, "va quantity symbol not allowed for selling" );
    }

    for (auto& method : pay_methods) {
        CHECKC( conf.pay_type.count(method) != 0,err::PARAM_ERROR, "pay method illegal: " + method.to_string() );
    }

    CHECKC( va_quantity.amount > 0,err::PARAM_ERROR, "quantity must be positive");
    // TODO: min order quantity
    CHECKC( va_price.amount > 0,err::PARAM_ERROR, "va price must be positive" );
    // TODO: price range
    CHECKC( va_min_take_quantity.symbol == va_quantity.symbol,err::PARAM_ERROR, "va_min_take_quantity Symbol mismatch with quantity" );
    CHECKC( va_max_take_quantity.symbol == va_quantity.symbol,err::PARAM_ERROR, "va_max_take_quantity Symbol mismatch with quantity" );
    CHECKC( va_min_take_quantity.amount > 0 && va_min_take_quantity.amount <= va_quantity.amount,err::PARAM_ERROR,
        "invalid va_min_take_quantity amount" );
    CHECKC( va_max_take_quantity.amount > 0 && va_max_take_quantity.amount <= va_quantity.amount,err::PARAM_ERROR,
        "invalid va_max_take_quantity amount" );

    merchant_t merchant(owner);
    CHECKC( _dbc.get(merchant),err::PARAM_ERROR, "merchant not found: " + owner.to_string() );
    CHECKC((merchant_status_t)merchant.status >= merchant_status_t::BASIC,err::PARAM_ERROR,
...
void otcbook::pauseorder(const name& owner, const name& order_side, const uint64_t& order_id) {
    ...
    CHECKC( order_wrapper_ptr != nullptr,err::PARAM_ERROR, "order not found");
    const auto &order = order_wrapper_ptr->get_order();
    CHECKC( owner == order.owner,err::PARAM_ERROR, "have no access to close others' order");
    CHECKC( (order_status_t)order.status == order_status_t::RUNNING,err::PARAM_ERROR, "order not running" );
...
void otcbook::resumeorder(const name& owner, const name& order_side, const uint64_t& order_id) {
    ...
    CHECKC( ORDER_SIDES.count(order_side) != 0, err::RECORD_NOT_FOUND, "Invalid order side" );
    ...
    CHECKC( (order_status_t)order.status == order_status_t::PAUSED, err::STATE_MISMATCH, "order not paused" );
    order_wrapper_ptr->modify(_self, [&]( auto& row ) {
...
void otcbook::closeorder(const name& owner, const name& order_side, const uint64_t& order_id) {
    ...
    const auto &order = order_wrapper_ptr->get_order();
    CHECKC( owner == order.owner, err::NO_AUTH, "have no access to close others' order");
    CHECKC( (uint8_t)order.status != (uint8_t)order_status_t::CLOSED, err::STATE_MISMATCH, "order already closed" );
    CHECKC( order.va_frozen_quantity.amount == 0, err::QUANTITY_INVALID, "order being processed" );
    CHECKC( order.va_quantity >= order.va_fulfilled_quantity, err::QUANTITY_INVALID, "order quantity insufficient" );
...
void otcbook::opendeal( const name& taker, const name& order_side, const uint64_t& order_id,
    ...
    if(order_side == BUY_SIDE) {
        CHECKC(deal_quantity.symbol != USDTARC_SYMBOL, err::SYMBOL_MISMATCH,  "deal quantity must not USDTARC_SYMBOL") 
    }
...
void otcbook::_opendeal( const name& taker, const name& order_side, const uint64_t& order_id,
    ...
    auto conf = _conf();
    CHECKC( conf.status == conf_status::RUNNING, err::STATE_MISMATCH, "service is in maintenance" );
    CHECKC( ORDER_SIDES.count(order_side) != 0, err::PARAM_ERROR, "Invalid order side" );
    ...
    CHECKC( order.status == (uint8_t)order_status_t::RUNNING, err::PAUSED, "order not running" );
    CHECKC( order.va_quantity >= order.va_frozen_quantity + order.va_fulfilled_quantity + deal_quantity,
                err::QUANTITY_INVALID, "Order's quantity insufficient" );
    CHECKC( deal_quantity >= order.va_min_take_quantity, err::QUANTITY_INVALID, "Order's min accept quantity not met!" );
    CHECKC( deal_quantity <= order.va_max_take_quantity, err::QUANTITY_INVALID, "Order's max accept quantity not met!" );

    auto now                    = current_time_point();

    blacklist_t::idx_t blacklist_tbl( _self, _self.value );
    auto blacklist_itr          = blacklist_tbl.find(taker.value);
    CHECKC( blacklist_itr == blacklist_tbl.end() || blacklist_itr->expired_at <= now,err::PAUSED, "taker is blacklisted" )
    ...
    CHECKC( ordersn_index.find(order_sn) == ordersn_index.end() ,err::RECORD_NOT_FOUND, "order_sn already existing!" );
...
deal_t otcbook::_closedeal(const name& account, const uint8_t& account_type, const uint64_t& deal_id, const string& close_msg, const bool& by_transfer) {
    ...
    CHECKC( deal_itr != deals.end(),err::RECORD_NOT_FOUND, "deal not found: " + to_string(deal_id) );
    auto status = (deal_status_t)deal_itr->status;
    CHECKC( (uint8_t)status != (uint8_t)deal_status_t::CLOSED,err::STATE_MISMATCH, "deal already closed: " + to_string(deal_id) );
    CHECKC( (uint8_t)status != (uint8_t)deal_status_t::CANCELLED,err::STATE_MISMATCH, "deal already cancelled: " + to_string(deal_id) );
    ...
    case account_type_t::MERCHANT:
        ...
        CHECKC( (uint8_t)status == (uint8_t)deal_status_t::MAKER_RECV_AND_SENT || 
            (uint8_t)status == (uint8_t)deal_status_t::TAKER_SENT,err::STATE_MISMATCH, "can only close deal in status taker_sent or maker_recv");
        ...
        break;
    default:
        CHECKC(false, err::NO_AUTH, "account type not supported: " + to_string(account_type));
        break;
    }
    ...
    CHECKC( order.va_frozen_quantity >= deal_quantity,err::QUANTITY_INVALID, "Err: order frozen quantity smaller than deal quantity" );
    ...
    CHECKC( _dbc.get(merchant), err::RECORD_NOT_FOUND,"merchant not found: " + order_maker.to_string() );
...
void otcbook::canceldeal(const name& account, const uint8_t& account_type, const uint64_t& deal_id, bool is_taker_black) {
    ...
    case account_type_t::USER:
        switch ((deal_status_t) status) {
            ...
            default:
                CHECKC( false, err::PARAM_ERROR,  "deal status need be CREATED or MAKER_ACCEPTED, deal_id:" + to_string(deal_id));
        }
...
deal_t otcbook::_process(const name& account, const uint8_t& account_type, const uint64_t& deal_id, uint8_t action_type) {
    ...
    CHECKC( status != deal_status_t::CLOSED, err::STATE_MISMATCH,"deal already closed: " + to_string(deal_id) );
    CHECKC( status != deal_status_t::CANCELLED,err::STATE_MISMATCH, "deal already cancelled: " + to_string(deal_id) );
...
#define DEAL_ACTION_CASE(_action, _limited_account_type, _limit_arbit_status,  _limited_status, _next_status) \
    ...
    if (limited_status != deal_status_t::NONE)
        CHECKC(limited_status == status,err::STATE_MISMATCH, "can not process deal action:" + to_string((uint8_t)action_type)
             + " at status: " + to_string((uint8_t)status) );
    if (limited_account_type != account_type_t::NONE)
        CHECKC(limited_account_type == (account_type_t)account_type,err::STATE_MISMATCH,
    ...
    if ( (uint8_t)limit_arbit_status != (uint8_t)arbit_status_t::NONE)
        CHECKC(arbit_status == limit_arbit_status,err::STATE_MISMATCH,
            "can not process deal action:" + to_string((uint8_t)action_type)
...
void otcbook::startarbit(const name& account, const uint8_t& account_type, const uint64_t& deal_id) {
    ...
    auto status = (deal_status_t)deal_itr->status;
    auto arbit_status = (arbit_status_t)deal_itr->arbit_status;
    CHECKC( arbit_status == arbit_status_t::UNARBITTED , err::STATE_MISMATCH, "arbit already started: " + to_string(deal_id) );
    ...
    CHECKC( can_arbit_status.count(status) != 0, err::STATE_MISMATCH,"status illegal: " + to_string((uint8_t)status) );
...
void otcbook::closearbit(const name& account, const uint64_t& deal_id, const uint8_t& arbit_result) {
    ...
    const auto &order_maker  = deal_itr->order_maker;
    CHECKC( arbit_status == arbit_status_t::ARBITING, err::STATE_MISMATCH, "arbit isn't arbiting: " + to_string(deal_id) );
...
void otcbook::cancelarbit( const uint8_t& account_type, const name& account, const uint64_t& deal_id )
{
    ...
    CHECKC( arbit_status == arbit_status_t::ARBITING,err::STATE_MISMATCH, "deal is not arbiting" );
    auto status = deal_itr->status;

    switch ((account_type_t) account_type) {
    case account_type_t::MERCHANT:
        CHECKC( deal_itr->order_maker == account, err::NO_AUTH, "maker account mismatched");
        CHECKC( status == (uint8_t)deal_status_t::MAKER_ACCEPTED, err::STATE_MISMATCH,
...
void otcbook::resetdeal(const name& account, const uint64_t& deal_id){
    ...
    auto status = (deal_status_t)deal_itr->status;
    CHECKC( status != deal_status_t::CLOSED,err::STATE_MISMATCH, "deal already closed: " + to_string(deal_id) );
    CHECKC( status != deal_status_t::CREATED, err::STATE_MISMATCH, "deal no need to reverse" );
...
void otcbook::withdraw(const name& owner, asset quantity){
    ...
    CHECKC(state >= merchant_status_t::BASIC || state == merchant_status_t::DISABLED,err::STATE_MISMATCH, "merchant not enabled");
    ...
    CHECKC( (time_point_sec(current_time_point())-merchant.updated_at) > limit_seconds,err::TIME_NOT_EXPIRED,
...
void otcbook::_deposit(name from, name to, asset quantity, string memo) {
    ...
    CHECKC((merchant_status_t)merchant.status >= merchant_status_t::BASIC,err::STATE_MISMATCH,
        "merchant not enabled");
...
const fiat_conf_t& otcbook::_conf(bool refresh/* = false*/) {
    ...
    CHECKC( itr != conf_tbl.end(),err::RECORD_NOT_FOUND, "conf table not existed in contract: " + _gstate.conf_contract.to_string());
...
void otcbook::_sub_balance(merchant_t& merchant, const asset& quantity, const string & memo){
    CHECKC( merchant.assets[quantity.symbol].balance >= quantity.amount,err::QUANTITY_INVALID, "merchant stake balance quantity insufficient");
    merchant.assets[quantity.symbol].balance -= quantity.amount;
    merchant.updated_at = current_time_point();
    _dbc.set( merchant , get_self());
    if(memo.length() > 0) STAKE_CHANGED(merchant.owner, -quantity, memo);
...
void otcbook::_unfrozen(merchant_t& merchant, const asset& quantity){
    CHECKC( merchant.assets[quantity.symbol].frozen >= quantity.amount,err::QUANTITY_INVALID, "merchant stake frozen quantity insufficient");
    merchant.assets[quantity.symbol].frozen -= quantity.amount;
    merchant.assets[quantity.symbol].balance += quantity.amount;
    merchant.updated_at = current_time_point();
    _dbc.set( merchant , get_self());
...
void otcbook::_merchant_apply(name from, asset quantity, vector<string_view> memo_params) {
    ...
    CHECKC(merchant_name.size() < 20, err::PARAM_ERROR, "merchant_name size too large: " + to_string(merchant_name.size()) );
    CHECKC(merchant_detail.size() < 255,err::PARAM_ERROR, "merchant_detail size too large: " + to_string(merchant_detail.size()) );
    CHECKC(email.size() < 64,err::PARAM_ERROR, "email size too large: " + to_string(email.size()) );

    merchant_t merchant(from);
    CHECKC(!_dbc.get( merchant ),err::RECORD_FOUND,"merchant is existed");
...
void otcbook::addarbiter(const name& sender, const name& account, const string& email) {
    ...
    // CHECKC( !_dbc.get(arbiter), err::RECORD_FOUND, "arbiter already exists: " + account.to_string() );
    arbiter.email = email;
...
void otcbook::delarbiter(const name& sender, const name& account) {
    ...
    auto arbiter = arbiter_t(account);
    CHECKC( _dbc.get(arbiter), err::RECORD_FOUND, "arbiter not found: " + account.to_string() );
